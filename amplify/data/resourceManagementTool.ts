import { ConversationTurnEvent, createExecutableTool, ExecutableTool, handleConversationTurnEvent } from "@aws-amplify/backend-ai/conversation/runtime";
...
const makeResourceManagementTool = (event: ConversationTurnEvent): ExecutableTool<typeof resourceManagementSchema> => {
  return createExecutableTool(
    ...
    async (input) => {
      ...
      const { data: project, errors: getProjectErrors } = await client.models.Project.get({ id: projectId });
      if (!project) {
        throw new Error(`Project with ID ${projectId} not found. ${getProjectErrors?.map((error) => error.message).join('\n')}`);
      }

      // Verify employee exists
      const { data: engineer, errors: getEngineerErrors } = await client.models.Engineer.get({ id: engineerId });
      if (!engineer) {
        throw new Error(`Engineer with ID ${engineerId} not found. ${getEngineerErrors?.map((error) => error.message).join('\n')}`);
      }
