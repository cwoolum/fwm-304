      ...
      <Tabs
        ...
        items={[
          ...
          },
          {
            ...
            value: 'Projects',
            content: (
              <Collection items={projects} type="list" gap="1rem" padding="1rem">
                {(project) => (
                  <Card key={project.id}>
                    <Flex
                      direction="row"
                    ...
                    >
                      <Flex direction="column" gap="0.5rem">
                        ...
                        <Flex gap="0.5rem">
                          {project.requiredSkills?.filter(s => s).map((skill) => (
                            <Badge key={skill} variation="info">
                              {skill}
                            </Badge>
                          ))}
                      ...
                      <Badge variation={project.status === "IN_PROGRESS" ? "success" : project.status === "COMPLETED" ? "warning" : "info"}>
                        {project.status}
                      </Badge>
                    </Flex>
                    <Text variation="secondary">
